...
const { getEmployeesAndTasks, getEmployeesByDepartment, uploadFile,
  createTask, downloadFiles, updateTaskStatus, putTaskOnReview, commentFilesDownload } = require('../controllers/task.controller')

router.get('/', requireLogin, getEmployeesAndTasks)
  .get('/:id/files/download', downloadFiles)
  .get('/:comment/:id/files/download', commentFilesDownload)
